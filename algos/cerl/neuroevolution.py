...
class SSNE:
    ...
    def __init__(self, args):
        ...
        self.num_elites = int(args.elite_fraction*self.popn_size)
    ...
    def get_anchors(self, states, pop, net_inds, lineage_rank):
        ...
        if pop[0].stochastic: #Stochatsic Policies
            actions = []; x_ts = []; dists = []
            for net in pop:
                action, _, x_t, dist, _ = net.noisy_action(states, return_only_action=False)
            ...
            for i in range(len(net_inds)):
                for j in range(len(net_inds)):
                    div_matrix[i, j] = -pop[i].compute_log_prob(x_ts[j], actions[j], dists[i]).mean().item()
    ...
    def epoch(self, pop, net_inds, fitness_evals, migration, states):
