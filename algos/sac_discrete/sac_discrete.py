...
class SAC_Discrete(object):
    def __init__(self, args, model_constructor, gamma):
        ...
        self.actor = model_constructor.make_model('Gumbel_FF').to(device=self.device)
    ...
    def update_parameters(self, state_batch, next_state_batch, action_batch, reward_batch, done_batch):
        ...
        #action_batch = torch.eye(self.actor.num_actions)[action_batch.long()].squeeze(1).cuda()
        ...
        with torch.no_grad():
            ...
            #print(next_q_value.shape, min_qf_next_target.shape, qf2_next_target.shape, next_state_log_pi.shape)
        ...
        #print(qf1_next_target.shape, next_state_log_pi.shape, next_q_value.shape, qf1.shape)

        pi, log_pi, _,_,temp = self.actor.noisy_action(state_batch, return_only_action=False)
        self.compute_stats(log_pi, self.entropy)
        self.compute_stats(temp, self.temp)
